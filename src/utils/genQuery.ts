...
import { Repository, Brackets, Not, And, Like, MoreThan, LessThan, MoreThanOrEqual, LessThanOrEqual, } from 'typeorm';
...
export const genMultiRelations = (paths: string) => {
    ...
    let buffer = pathList.map(path => {
        ...
        if (path.split('.').length > 0) {
            path.split('.').filter(r => r !== '*').map((k, i, values) => {
                container = (container[k] = (i == values.length - 1 ? defaultValues : {}))
            });
            return tempObject
        }
...
function joinRelationsToQuery<TEntity>(urm: UniqueRelationMap, query: SelectQueryBuilder<TEntity>): void {
