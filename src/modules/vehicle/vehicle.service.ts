...
export class VehicleService {
  ...
  async findAll() {
    return await this.trsVehicleRepo.query(
      `select  srsis.status as status,srsis.id as status_id,srst.id as source_id,srst.[type] as source,min(smit.id) as images,min(df.id ) as img,
      max(ta.start_date) as startdate,df.description ,ssiav.attribute_value as license,ssi.name as item,ssi.id as item_id ,sss.name as spec,
      ...
      left join dbo.trs_activity_vehicle_driver tavd on tavd.vehicle = ssi.id 
      left join dbo.trs_activity ta on ta.id = tavd.activity 
      ...
      group by ssiav.attribute_name,srsis.status,srsis.id,df.description,ssiav.attribute_value,ssi.name,ssi.id,sss.name,sss.id,ss.id,ss.supply_name,srst.id,srst.[type]      
      `,
    );
  }
