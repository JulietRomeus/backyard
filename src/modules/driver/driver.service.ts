...
export class DriverService {
  ...
  async create(createDriverDto: CreateDriverDto) {
    ...
    const trs_driver_license_lists: trsDriverLicenseList[] =
      dataObj.trs_driver_license_lists?.map((rec: any) => {
  ...
  async findOne(id: any) {
    ...
    finalItem.is_busy =
      data?.trs_activity_vehicle_drivers?.some(
        (r) => r?.activity?.is_inprogress,
      ) || false;
  ...
  async update(id: any, updateDriverDto: UpdateDriverDto) {
    ...
    updateObj.unit_no = dataObj.unit_no;
  ...
  async importDriver(req, body) {
    ...
    const object = body.data.map(async (rec) => {
      ...
      if (typeof rec?.license_type_id1 == 'number')
        tempLicense.push({
          license_id: parseInt(rec?.license_type_id1),
          ...(rec?.license_issue_date1
            ? {
                issue_date: parse(
                  rec?.license_issue_date1,
                  formatString,
      ...
      if (typeof rec?.license_type_id2 == 'number')
        tempLicense.push({
          license_id: parseInt(rec?.license_type_id2),
          ...(rec?.license_issue_date2
            ? {
                issue_date: parse(
                  rec?.license_issue_date2,
                  formatString,
      ...
      if (haveUser) {
        ...
        cUser.status = 1;
        cUser.password =
          '$2b$10$KSa3KWogEEbDTV0Nh5OlmuTtjz.VivJ.Ls5F03PQqb5.44d4Gc.Yy';
